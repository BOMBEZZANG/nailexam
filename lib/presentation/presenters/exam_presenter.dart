...
import '../../data/models/score_data.dart';
...
class ExamPresenter extends BasePresenter<ExamView> {
  ...
  void _initializeExam() {
    ...
    if (_currentSession!.status == ExamStatus.paused) {
      view?.enablePauseResume(true);
    }
  ...
  void startExam({bool isPracticeMode = false}) {
    try {
      if (_currentSession == null) {
        _currentSession = _gameManager.startNewSession(isPractice: isPracticeMode);
      }
      
      _timerManager.startExamTimer();
      startPeriod(1);
      
      view?.showSuccess('Exam started');
    } catch (e) {
  ...
  void pauseExam() {
    if (_currentSession == null || !_currentSession!.isPracticeMode) {
      view?.showError('Cannot pause during actual exam');
      return;
    }
    ...
    Logger.i('Exam paused');
  }
  
  void resumeExam() {
    _gameManager.resumeSession();
    _timerManager.resume();
    view?.enablePauseResume(false);
    
    Logger.i('Exam resumed');
  }
  ...
  void _updatePeriodInfo() {
    ...
    if (periodData != null) {
      final periodName = ExamConstants.periodNames[periodData.periodNumber] ?? 'Unknown';
      final techniqueName = ExamConstants.techniqueDisplayNames[periodData.assignedTechnique] ?? 
